...
figure;
plot(N, ones(size(N)), '--k', 'LineWidth', 1.5); hold on; % 参考线
plot(N, speedup_p2, '-s', 'LineWidth', 2);
plot(N, speedup_p3, '-^', 'LineWidth', 2);
xlabel('矩阵规模 N');
ylabel('加速比');
title('不同版本加速比对比');
legend('理想加速比 (1)', 'p2.cpp (MPI+线程+SIMD)', 'p3.cpp (MPI非阻塞)');
grid on;
